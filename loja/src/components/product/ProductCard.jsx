...
export function ProductCard({ product }) {
  ...
  return (
    <div className="group glass-card rounded-2xl p-3 flex flex-col justify-between transition-all duration-300 hover:translate-y-[-4px] hover:border-brand-purple/40">
      ...
      <div className="mt-3 flex-1 flex flex-col">
        ...
        {/* Botões de Ação */}
        <div className="mt-4 grid grid-cols-2 gap-2 pt-3 border-t border-brand-border/60">
          <Link
            to={`/produto/${product.slug}`}
            className="inline-flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl bg-brand-surface hover:bg-brand-card text-slate-300 hover:text-white text-xs font-bold border border-brand-border hover:border-brand-purple/50 transition-colors"
          >
            <Eye className="w-3.5 h-3.5 text-brand-purpleLight" />
          ...
          <a
            ...
            className="inline-flex items-center justify-center gap-1.5 py-2 px-2.5 rounded-xl bg-brand-whatsapp text-white hover:bg-brand-whatsappHover text-xs font-extrabold shadow-md shadow-brand-whatsapp/20 transition-all active:scale-95"
