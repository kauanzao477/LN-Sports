...
export function ProductCard({ product }) {
  ...
  // Posição na ordem de tentativa da capa: começa na principal e, se ela
  // falhar (ex: placeholder do Yupoo barrado pelo proxy), avança sozinha
  // para a próxima foto válida em vez de exibir erro.
  const [coverPos, setCoverPos] = useState(0);

  useEffect(() => {
    setCoverPos(0);
  }, [product?.slug, product?.sourceUrl]);

  if (!product) return null;

  const mainIdx = (product.mainImageIndex !== undefined && product.mainImageIndex !== null)
    ? product.mainImageIndex
    : 0;
  const imgs = Array.isArray(product.images) ? product.images : [];
  const orderedIndexes = imgs.length > 0
    ? [mainIdx, ...imgs.map((_, i) => i).filter((i) => i !== mainIdx)].filter((i) => i >= 0 && i < imgs.length)
    : [];
  const activeIdx = orderedIndexes[Math.min(coverPos, Math.max(orderedIndexes.length - 1, 0))] ?? null;
  const mainImage = activeIdx !== null ? imgs[activeIdx] : null;
    ...
    <div className="group glass-card rounded-2xl p-2 sm:p-3 flex flex-col justify-between transition-all duration-300 hover:translate-y-[-4px] hover:border-brand-purple/40">
      ...
      <Link to={`/produto/${product.slug}`} className="block relative overflow-hidden rounded-xl">
        <ProductImage
          ...
          onError={() => {
            if (coverPos < orderedIndexes.length - 1) setCoverPos((p) => p + 1);
          }}
        />
