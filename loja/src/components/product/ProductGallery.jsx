...
export function ProductGallery({
  ...
  // Índices de fotos que falharam (ex: placeholder do Yupoo barrado pelo
  // proxy). A galeria pula essas fotos sozinha para a próxima válida.
  ...
  // Se a foto atual falhou e existe outra válida, avança sozinho
  useEffect(() => {
    if (validImages.length === 0) return;
    if (!failedIndexes.has(selectedIndex)) return;
    const usable = validImages.map((_, i) => i).filter((i) => !failedIndexes.has(i));
    if (usable.length === 0) return; // todas falharam: exibe fallback
    const next = usable.find((i) => i > selectedIndex) ?? usable[0];
    if (next !== selectedIndex) setSelectedIndex(next);
  }, [failedIndexes, selectedIndex, validImages.length]);
