...
def extract_photo_hash(url: str) -> str:
    """
    ...
    (og:image .../<hash>/medium.jpg == original .../<hash>/<arquivo>.jpg).
    Retorna '' se não for foto de produto válida (logos /icons/, ids curtos).
    Mesma regra do frontend em loja/src/utils/coverUtils.js.
    """
    if not url or not isinstance(url, str):
        return ""
    try:
        parsed = urllib.parse.urlparse(url.strip())
        if 'yupoo.com' not in (parsed.hostname or ''):
            return ""
        ...
        if parts[0] == 'icons':
            return ""
        photo_hash = parts[1]
        if not re.fullmatch(r'[a-fA-F0-9]{6,}', photo_hash):
            return ""
        return photo_hash.lower()
    except Exception:
        return ""
...
class ProductExtractor:
    ...
    def extract_yupoo_album(
        ...
        # 2.1 CAPA OFICIAL DO ÁLBUM (foto do tênis EM PAR, escolhida pelo vendedor).
        # O Yupoo expõe essa capa na tag og:image. O hash identifica a mesma foto
        # em qualquer resolução (medium.jpg == original), então casamos por hash
        # em vez de usar a URL de miniatura diretamente.
        cover_image = ""
        cover_hash = ""
        og_tag = soup.find('meta', property='og:image')
        if og_tag and og_tag.get('content'):
            candidate = og_tag.get('content').strip()
            candidate_hash = extract_photo_hash(candidate)
            if candidate_hash:
                cover_image = candidate
                cover_hash = candidate_hash
                if debug:
                    debug_log.append(f"[DEBUG] Capa oficial (og:image): hash {cover_hash}")
    ...
    def extract_generic_album(
        ...
        # Capa oficial (og:image), quando disponível
        cover_image = ""
        cover_hash = ""
        og_tag = soup.find('meta', property='og:image')
        if og_tag and og_tag.get('content'):
