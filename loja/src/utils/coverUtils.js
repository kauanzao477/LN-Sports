 ...
 */
export function resolveCoverIndex(product, adminIndex = null) {
  try {
    const images = product?.images;
    if (!Array.isArray(images) || images.length === 0) return 0;

    // 1. Escolha manual do admin tem prioridade absoluta
    if (Number.isInteger(adminIndex) && adminIndex >= 0 && adminIndex < images.length) {
      return adminIndex;
    }

    // 2. Índice já gravado (scraper novo / backfill)
    const stored = product?.mainImageIndex;
    if (isValidCoverIndex(stored, images)) return stored;

    // 3. Capa oficial do álbum (foto do par) — só faz sentido com hash válido
    //    e a foto precisa existir na lista atual (admin pode ter reordenado).
    const coverHash = product?.coverHash;
    if (coverHash) {
      const matched = findCoverIndex(images, coverHash);
